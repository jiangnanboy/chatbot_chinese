...
class Decoder(nn.Module):
    def __init__(self, output_dim, emb_dim, hidden_dim, n_layers, dropout, attention_method='dot'):
...
class Seq2Seq(nn.Module):
    ...
    def forward(self, src, src_lens, trg, teacher_forcing_ration=0.8, max_len=50):
...
def train(model, iterator, optimizer, criterion, clip):
    ...
    for i, batch in enumerate(iterator):
        src, src_lens = batch.src  # src=[batch_size, seq_len]，这里batch.src返回src和src的长度，因为在使用torchtext.Field时设置include_lengths=True
        trg, _ = batch.trg  # trg=[batch_size, seq_len]
        src = src.to(device)
        trg = trg.to(device)
        output = model(src, src_lens, trg, teacher_forcing_ration=1)  # output=[batch_size, seq_len, output_dim]
...
def evaluate(model, iterator, criterion):
    ...
    with torch.no_grad():  # 不更新梯度
        for i, batch in enumerate(iterator):
            src, src_len = batch.src  # src=[batch_size, seq_len]
            trg, _ = batch.trg  # trg=[batch_size, seq_len]
            src = src.to(device)
            trg = trg.to(device)
            # output=[batch_size, seq_len, output_dim]
            output = model(src, src_len, trg, teacher_forcing_ration=0)  # 评估的时候不使用teacher force，使用预测作为每一步的输入
...
def predict(sentence, vocab, model):
    with torch.no_grad():
        ...
        prediction = model(tensor, [len(tensor)], None)  # prediction
        return prediction
...
if __name__ == '__main__':
    ...
    if args.type == 'train':
        ...
        train_model(model,
                    ...
                    config.get(section, 'seq2seq_attention_model'))
        ...
        model.load_state_dict(torch.load(config.get(section, 'seq2seq_attention_model')))
        model.eval()
        while True:
            sentence = input('you:')
            if sentence == 'exit':
